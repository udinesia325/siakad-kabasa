...
export default function JurnalBuatSlot({ mode, jadwal, siswa: initialSiswa, jurnal_id }: Props) {
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                {[jadwal.tingkat, jadwal.kelas].filter(Boolean).join(' ')} · {jadwal.jam_mulai}–{jadwal.jam_selesai} · {tanggalFormatted}
                            </p>
                        </div>
                        {mode === 'edit' && (
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-200 dark:border-slate-700">
                                ...
                                {STATUS_LABELS.map(s => (
                                    <th key={s} className={`px-3 py-3 text-center text-xs font-semibold uppercase tracking-wider ${STATUS_COLOR[s]}`}>
                                        {STATUS_SHORT[s]}
                                    </th>
                                ))}
