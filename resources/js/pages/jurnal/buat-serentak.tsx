...
const STATUS_LABELS: StatusJurnal[] = ['hadir', 'alpha', 'sakit', 'izin', 'dispensasi'];

const STATUS_SHORT: Record<StatusJurnal, string> = {
    hadir: 'H', alpha: 'A', sakit: 'S', izin: 'I', dispensasi: 'D',
};
...
type SiswaJurnal = { id: number; nama: string; status: StatusJurnal; keterangan: string | null };
...
type Props = {
    jadwals: JadwalSlotInfo[];
    info: InfoSerentak;
    siswa: SiswaJurnal[];
};

export default function JurnalBuatSerentak({ jadwals, info, siswa: initialSiswa }: Props) {
    ...
    function setStatus(siswaId: number, status: StatusJurnal) {
        setDetails((prev) => prev.map((d) => (d.id === siswaId ? { ...d, status } : d)));
    }

    function setKeterangan(siswaId: number, keterangan: string) {
        setDetails((prev) => prev.map((d) => (d.id === siswaId ? { ...d, keterangan } : d)));
    }

    function handleReset() {
        setDetails(initialSiswa.map((s) => ({ ...s, keterangan: s.keterangan ?? '' })));
    }
    ...
    const tanggalFormatted = new Date(info.tanggal).toLocaleDateString('id-ID', {
        weekday: 'long', day: 'numeric', month: 'long', year: 'numeric',
    });

    return (
        <>
            <Head title={`Jurnal Serentak — ${info.mata_pelajaran}`} />
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                                ...
                                <span>
                                    {info.kelas} · Jam {info.nomor_jam_dari}–{info.nomor_jam_sampai} · {info.jam_mulai}–{info.jam_selesai} · {tanggalFormatted}
                                </span>
                            </div>
                            <div className="mt-2 flex flex-wrap gap-1.5">
                                {jadwals.map((j) => (
                                    ...
                                    >
                                        Jam {j.nomor_jam} · {j.jam_mulai}–{j.jam_selesai}
                                    </span>
                                ))}
                            </div>
                        </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-200 dark:border-slate-700">
                                ...
                                {STATUS_LABELS.map((s) => (
                                    <th key={s} className={`px-3 py-3 text-center text-xs font-semibold tracking-wider uppercase ${STATUS_COLOR[s]}`}>
                                        {STATUS_SHORT[s]}
                                    </th>
                                ))}
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700/60">
                            {details.map((siswa, idx) => (
                                <tr key={siswa.id} className="group hover:bg-slate-50 dark:hover:bg-slate-700/30">
                                    ...
                                    <td className="px-4 py-3">
                                        <input
                                            ...
                                            onChange={(e) => setKeterangan(siswa.id, e.target.value)}
                            ...
                            <AlertDialogContent size="sm">
                                <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                        Semua pilihan status dan keterangan akan dikembalikan ke kondisi awal.
                                    </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
