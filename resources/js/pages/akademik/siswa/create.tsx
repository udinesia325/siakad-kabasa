...
export default function SiswaCreate({ kelas }: Props) {
    const form = useForm({
        nik: '', nis: '', nama: '', jenis_kelamin: '' as 'L' | 'P',
        email: '', alamat: '', kelas_id: '' as string,
    });
                ...
                <form onSubmit={submit} className="flex flex-col gap-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-2">
                            ...
                            <Input value={form.data.nik} onChange={(e) => form.setData('nik', e.target.value)} placeholder="16 digit NIK" />
                            {form.errors.nik && <p className="text-sm text-destructive">{form.errors.nik}</p>}
                        </div>
                        <div className="flex flex-col gap-2">
                            <Label>NIS <span className="text-muted-foreground text-xs">(opsional)</span></Label>
                            <Input value={form.data.nis} onChange={(e) => form.setData('nis', e.target.value)} placeholder="Nomor Induk Siswa" />
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <Input value={form.data.nama} onChange={(e) => form.setData('nama', e.target.value)} placeholder="Nama lengkap siswa" />
                    ...
                    <div className="flex flex-col gap-2">
                        <Label>Email</Label>
                        <Input type="email" value={form.data.email} onChange={(e) => form.setData('email', e.target.value)} placeholder="email@sekolah.sch.id" />
                        {form.errors.email && <p className="text-sm text-destructive">{form.errors.email}</p>}
                    </div>

                    <div className="flex flex-col gap-2">
                        <Label>Alamat</Label>
                        <Textarea value={form.data.alamat} onChange={(e) => form.setData('alamat', e.target.value)} placeholder="Alamat lengkap" />
