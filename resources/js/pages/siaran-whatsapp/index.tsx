...
function PilihKelas({
    ...
    kelas: Kelas[];
    onPilih: (k: Kelas) => void;
}) {
    return (
        <div>
            <p className="mb-6 text-sm text-muted-foreground">
                Pilih kelas untuk mengirim pesan siaran.
            </p>
                    ...
                    <CardContent className="flex flex-col items-center gap-3 py-8">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-xs text-red-500">
                                Kirim pesan ke orang tua siswa yang tidak hadir
                                hari ini
                            </p>
                        </div>
                    </CardContent>
                    ...
                    <CardContent className="flex flex-col items-center gap-3 py-8">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-xs text-yellow-600">
                                Kirim pesan ke orang tua siswa yang terlambat
                                hari ini
                            </p>
                        </div>
                    </CardContent>
...
function ListSiswa({
    ...
    const allChecked =
        siswaWithPhone.length > 0 &&
        siswaWithPhone.every((s) => checkedIds.has(s.id));
        ...
        <div>
            <div className="mb-4 flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                    {checkedIds.size} siswa dipilih dari {siswaList.length}{' '}
                    siswa
                </span>
            </div>
                        ...
                        return (
                            ...
                            >
                                <Checkbox
                                    id={`siswa-${s.id}`}
                                    checked={
                                        hasPhone ? checkedIds.has(s.id) : false
                                    }
                                    disabled={!hasPhone}
                                    onCheckedChange={() =>
                                        hasPhone && onToggle(s.id)
                                    }
                                />
                                <label
                                    htmlFor={`siswa-${s.id}`}
                                ...
                                >
                                    <span
                                        className={`text-sm font-medium ${!hasPhone ? 'text-muted-foreground' : ''}`}
                                    >
                                        {s.nama}
                                    </span>
                                    {hasPhone ? (
        ...
        <div className="flex flex-col items-center gap-4 py-12 text-center">
            ...
            <div>
                ...
                <p className="text-sm text-muted-foreground">
                    Pesan telah dikirimkan ke {jumlah} orang tua siswa.
                </p>
            </div>
...
export default function SiaranWhatsappIndex({ kelas }: Props) {
    ...
    function handleToggleAll() {
        const withPhone = siswaList
            .filter((s) => s.no_telepon !== null)
            .map((s) => s.id);
