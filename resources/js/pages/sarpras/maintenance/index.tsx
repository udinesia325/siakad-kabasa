...
type Props = {
    maintenances: { data: Maintenance[]; links: any[]; meta: any; current_page: number; last_page: number; total: number };
...
export default function MaintenanceIndex({ maintenances, barangs, vendors, filters }: Props) {
    ...
    useEffect(() => {
        ...
        const handle = setTimeout(() => {
            router.get('/sarpras/maintenance', {
    ...
    function openEdit(item: Maintenance) {
        editForm.setData({
            ...
            tgl_rencana: item.tgl_rencana ? item.tgl_rencana.substring(0, 10) : '',
            tgl_selesai: item.tgl_selesai ? item.tgl_selesai.substring(0, 10) : '',
    ...
    const currentPage = (maintenances as any).current_page ?? maintenances.meta?.current_page ?? 1;
    const lastPage = (maintenances as any).last_page ?? maintenances.meta?.last_page ?? 1;
        ...
        <>
            ...
            <div className="flex flex-col gap-4 p-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-semibold">Jadwal Maintenance</h1>
                    {canCreate && (
                        ...
                        <TableBody>
                            ...
                            {data.map((item) => (
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell className="text-sm">{item.vendor?.nama ?? '—'}</TableCell>
                                    <TableCell className="text-sm">{formatDate(item.tgl_rencana)}</TableCell>
                                    <TableCell className="text-sm">{formatDate(item.tgl_selesai)}</TableCell>
                                    <TableCell>
                                        <Badge variant="outline">{INTERVAL_LABEL[item.interval]}</Badge>
                                    </TableCell>
                                    <TableCell className="text-sm">{formatIDR(item.biaya)}</TableCell>
                                    <TableCell>
            ...
            <Dialog open={openCreate} onOpenChange={setOpenCreate}>
                <DialogContent className="max-w-lg">
                    <DialogHeader><DialogTitle>Tambah Jadwal Maintenance</DialogTitle></DialogHeader>
                    <form onSubmit={submitCreate} className="flex flex-col gap-4">
                        <MaintenanceFormFields form={createForm} barangs={barangs} vendors={vendors} />
                        <DialogFooter>
            ...
            <Dialog open={!!editTarget} onOpenChange={(v) => !v && setEditTarget(null)}>
                <DialogContent className="max-w-lg">
                    <DialogHeader><DialogTitle>Edit Jadwal Maintenance</DialogTitle></DialogHeader>
                    <form onSubmit={submitEdit} className="flex flex-col gap-4">
                        <MaintenanceFormFields form={editForm} barangs={barangs} vendors={vendors} />
                        <DialogFooter>
            ...
            <AlertDialog open={!!deleteTarget} onOpenChange={(v) => !v && setDeleteTarget(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Hapus jadwal maintenance?</AlertDialogTitle>
                        <AlertDialogDescription>
                            Jadwal maintenance untuk <strong>{deleteTarget?.barang?.nama}</strong> akan dihapus.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2 flex flex-col gap-2">
                    ...
                    <Select
                        ...
                        onValueChange={(v) => form.setData('barang_id', v === 'none' ? '' : v)}
                    >
                    ...
                    {form.errors.barang_id && <p className="text-sm text-destructive">{form.errors.barang_id}</p>}
                </div>
                ...
                <div className="flex flex-col gap-2">
                    <Label>Tgl Rencana <span className="text-destructive">*</span></Label>
                    <Input
                        type="date"
                        value={form.data.tgl_rencana}
                        onChange={(e) => form.setData('tgl_rencana', e.target.value)}
                    />
                    {form.errors.tgl_rencana && <p className="text-sm text-destructive">{form.errors.tgl_rencana}</p>}
                </div>
                ...
                <div className="flex flex-col gap-2">
                    <Label>Interval <span className="text-destructive">*</span></Label>
                    <Select
                        value={form.data.interval}
                        onValueChange={(v) => form.setData('interval', v as Interval)}
                    >
                    ...
                    {form.errors.interval && <p className="text-sm text-destructive">{form.errors.interval}</p>}
                </div>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Select
                        ...
                        onValueChange={(v) => form.setData('status', v as StatusMaintenance)}
                    >
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                            {Object.entries(STATUS_LABEL).map(([k, v]) => (
                ...
                <div className="col-span-2 flex flex-col gap-2">
                    ...
                    <Input
                        ...
                        placeholder="0"
                    />
                    {form.errors.biaya && <p className="text-sm text-destructive">{form.errors.biaya}</p>}
                </div>
