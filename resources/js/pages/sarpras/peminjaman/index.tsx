...
import { CheckCircle, PlusCircle, RotateCcw, Trash2, XCircle } from 'lucide-react';
...
export default function PeminjamanIndex({ peminjaman, filters, barangList, authId }: Props) {
    const [open, setOpen] = useState(false);
    const [deleteTarget, setDeleteTarget] = useState<Peminjaman | null>(null);
    const [actionTarget, setActionTarget] = useState<{ item: Peminjaman; action: 'approve' | 'reject' | 'kembalikan' } | null>(null);
    const [search, setSearch] = useState(filters.search ?? '');
    const [status, setStatus] = useState(filters.status ?? 'all');
    const isFirstRender = useRef(true);

    const { auth } = usePage<{ auth: { is_superadmin: boolean; permissions: string[] } }>().props;
    const canApprove = auth.is_superadmin || auth.permissions.some((p) => p.startsWith('sarpras.peminjaman.approve'));
...
return;
        ...
        router.post(`/sarpras/peminjaman/${item.id}/${action}`, {}, {
            preserveScroll: true,
        ...
        <>
            ...
            <div className="flex flex-col gap-4 p-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-semibold">Peminjaman Barang</h1>
                    <Button
                        onClick={() => {
                            form.reset();
                            setOpen(true);
                        ...
                        <TableBody>
                            ...
                            {peminjaman.data.map((item) => (
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell className="text-sm">{item.peminjam?.name ?? '—'}</TableCell>
                                    <TableCell className="text-sm">{item.tgl_pinjam}</TableCell>
                                    <TableCell className="text-sm">
                                        <div>{item.tgl_kembali_rencana}</div>
                                        {item.tgl_kembali_aktual && (
                                            <div className="text-xs text-muted-foreground">Aktual: {item.tgl_kembali_aktual}</div>
                                        )}
                                    </TableCell>
                                    <TableCell>
                                        ...
                                        <div className="flex gap-1">
                                            ...
                                            {canApprove && item.status === 'disetujui' && (
                                                <Button
                                                    ...
                                                    variant="ghost"
                                                    title="Catat Kembali"
                                                    onClick={() => setActionTarget({ item, action: 'kembalikan' })}
                                                >
                                                    <RotateCcw className="h-4 w-4 text-blue-600" />
                                                </Button>
                                            )}
                                            {item.peminjam_id === authId && item.status === 'menunggu' && (
                                                <Button size="icon" variant="ghost" onClick={() => setDeleteTarget(item)} title="Batalkan">
                ...
                {peminjaman.last_page > 1 && (
                    <p className="text-sm text-muted-foreground">
                        Halaman {peminjaman.current_page} dari {peminjaman.last_page} · {peminjaman.total} total
                    </p>
                )}
            </div>
                    ...
                    <form onSubmit={submit} className="flex flex-col gap-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="flex flex-col gap-2">
                                ...
                                <Input type="date" value={form.data.tgl_pinjam} onChange={(e) => form.setData('tgl_pinjam', e.target.value)} />
                                {form.errors.tgl_pinjam && <p className="text-sm text-destructive">{form.errors.tgl_pinjam}</p>}
                            </div>
                            <div className="flex flex-col gap-2">
                                ...
                                <Input
                                    ...
                                    onChange={(e) => form.setData('tgl_kembali_rencana', e.target.value)}
                                />
                                {form.errors.tgl_kembali_rencana && (
                                    <p className="text-sm text-destructive">{form.errors.tgl_kembali_rencana}</p>
                                )}
                            </div>
                        </div>
                        <div className="flex flex-col gap-2">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <Textarea rows={2} value={form.data.catatan} onChange={(e) => form.setData('catatan', e.target.value)} />
                        </div>
                        <DialogFooter>
            ...
            <AlertDialog open={!!actionTarget} onOpenChange={(v) => !v && setActionTarget(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Barang: <strong>{actionTarget?.item.barang?.nama}</strong> oleh {actionTarget?.item.peminjam?.name}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
